...
def test_browse_button_commits_sprite_dir_without_extra_interaction(qtbot, tmp_path, monkeypatch):
    ...
    # No Enter press, no focus change - the model must already reflect the pick.
    assert project.layers[0].sprite_dir == str(new_dir)
    assert panel.sprite_dir_edit.text() == str(new_dir)
...
def test_car_browse_button_commits_sprite_dir_without_extra_interaction(qtbot, tmp_path, monkeypatch):
    ...
    assert project.cars[0].sprite_dir == str(new_dir)
    assert panel.car_sprite_dir_edit.text() == str(new_dir)
